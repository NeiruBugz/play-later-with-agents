...
class CollectionService:
    ...
    def get_collection_stats(self, *, current_user: CurrentUser) -> CollectionStats:
        base_query = (
            select(CollectionItem, Game)
            .join(Game, CollectionItem.game_id == Game.id)
            .where(CollectionItem.user_id == current_user.id)
    ...
    def create_collection_item(
        self, *, current_user: CurrentUser, item_data: CollectionItemCreate
    ) -> CollectionItemExpanded:
        game_query = select(Game).where(Game.id == item_data.game_id)
        game = self.db.scalar(game_query)
        if not game:
    ...
    def delete_collection_item(
        ...
        collection_item = self.db.scalar(query)
        ...
        if hard_delete:
            ...
            self.db.commit()
            return {
                "message": "Collection item permanently deleted",
                "id": collection_id,
            }
    ...
    def bulk_collection_operations(
        ...
        for collection_id in request.collection_ids:
            try:
                collection_item = self.db.scalar(
                ...
                if request.action == BulkCollectionAction.UPDATE_PRIORITY:
                    ...
                    platform_val = request.data["platform"]  # type: ignore[index]
                    if not platform_val or not isinstance(platform_val, str):
                        results.append(
                            BulkCollectionResult(
                                id=collection_id,
                                success=False,
                        ...
                        continue
                    collection_item.platform = platform_val
                    updated_data["platform"] = platform_val
