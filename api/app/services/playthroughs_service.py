...
class PlaythroughsService:
    ...
    def list_playthroughs(
        ...
        *,
        ...
        filters = []
        if status_:
            filters.append(Playthrough.status.in_([s.value for s in status_]))
        if platform:
    ...
    def create_playthrough(
        self, *, current_user: CurrentUser, playthrough_data: PlaythroughCreate
    ) -> PlaythroughResponse:
        game = self.db.query(Game).filter(Game.id == playthrough_data.game_id).first()
        if not game:
