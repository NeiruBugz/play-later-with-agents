...
def test_list_collection_with_is_active_filter(test_data):
    ...
    response = client.get(
        "/api/v1/collection?is_active=true",
        headers={"X-User-Id": "user1"}
    )
    assert response.status_code == 200
    data = response.json()
    ...
    response = client.get(
        "/api/v1/collection?is_active=false",
        headers={"X-User-Id": "user1"}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_list_collection_with_platform_filter(test_data):
    ...
    response = client.get(
        "/api/v1/collection?platform=PS5",
        headers={"X-User-Id": "user1"}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_list_collection_with_acquisition_type_filter(test_data):
    ...
    response = client.get(
        "/api/v1/collection?acquisition_type=DIGITAL",
        headers={"X-User-Id": "user1"}
    )
    assert response.status_code == 200
    data = response.json()
    ...
    response = client.get(
        "/api/v1/collection?acquisition_type=PHYSICAL",
        headers={"X-User-Id": "user1"}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_list_collection_with_search(test_data):
    ...
    response = client.get(
        "/api/v1/collection?search=witcher",
        headers={"X-User-Id": "user1"}
    )
    assert response.status_code == 200
    data = response.json()
    ...
    # Search in notes
    response = client.get(
        "/api/v1/collection?search=souls-like",
        headers={"X-User-Id": "user1"}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_list_collection_with_playthroughs(test_data):
    ...
    witcher_item = next(
        ...
        if item["game"]["title"] == "The Witcher 3"
    )
...
def test_list_collection_invalid_parameters(test_data):
    ...
    # Invalid sort_order
    response = client.get(
        "/api/v1/collection?sort_order=invalid",
        headers={"X-User-Id": "user1"}
    )
